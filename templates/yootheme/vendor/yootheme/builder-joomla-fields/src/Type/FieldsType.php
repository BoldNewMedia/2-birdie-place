...
class FieldsType
{
        ...
        return [
            ...
            'fields' => array_filter(array_reduce($fields, function ($fields, $field) use ($source, $type, $context) {
                ...
                if (is_callable($callback = [__CLASS__, "config{$field->type}String"])) {
                    $config = $callback($field, $config, $source);
                    $fields[$field->name . 'String'] = Event::emit('source.com_fields.field|filter', $config, $field, $source, $context);
                }
    ...
    public function resolveSql($field)
    {
        ...
        $db->setQuery('SELECT value, text FROM (' . $query . ') as a having value in (' . trim($condition, ',') . ')');
