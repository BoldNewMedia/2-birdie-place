...
class CustomizerListener
{
    ...
    {
        $notFoundRoute = Url::route('/builder/template/404');

        $base = Uri::root(true) . '/administrator';
        if (Str::startsWith($notFoundRoute, $base)) {
            $notFoundRoute = Uri::root(true) . Str::substr($notFoundRoute, strlen($base));
        }

        $config->set('customizer.404', $notFoundRoute);
