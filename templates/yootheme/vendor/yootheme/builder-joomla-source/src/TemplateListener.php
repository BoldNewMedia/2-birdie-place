...
class TemplateListener
{
    ...
    public static function matchTemplate(Document $document, $view, $tpl)
    {
        ...
        if ($context === 'com_tags.tag') {
            ...
            return [
                ...
                'params' => ['tags' => $view->get('item'), 'items' => $view->get('items'), 'pagination' => $pagination],
            ];
        }
    ...
    public static function load404(Application $app, CMSApplication $cms, Config $config, $event)
    {
        ...
        $v = new HtmlView(['name' => '404', 'base_path' => '']);
        $cms->triggerEvent('onLoadTemplate', [$v, null]);

        $result['customizer'] = sprintf('<script>var $customizer = JSON.parse(atob("%s"));</script>', base64_encode(json_encode($config('customizer'))));
