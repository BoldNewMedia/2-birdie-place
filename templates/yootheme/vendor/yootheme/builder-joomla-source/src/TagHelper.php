...
class TagHelper
{
    ...
    public static function get($ids)
    {
        if (!class_exists('TagsModelTag')) {
            require_once JPATH_ROOT . '/components/com_tags/models/tag.php';
    ...
    {
        $model = new TagsModelTags(['ignore_request' => true]);
    ...
    public static function filterTags($tags, $parentId)
    {
        ...
        return $parent ? array_filter($tags, function ($tag) use ($parent) {
...
if (!class_exists('TagsModelTags')) {
    require_once JPATH_ROOT . '/components/com_tags/models/tags.php';
}

class TagsModelTags extends \TagsModelTags
{
    protected function getListQuery()
    {
        ...
        if ($this->state->params->get('all_tags_orderby', 'title') == 'rand') {
            $query->clear('order');
            $query->order(Factory::getDbo()->getQuery(true)->Rand());
        }
