...
class ContentListener
{
    ...
    {
        if (static::isArticleView() and $config('req.customizer.admin') and $user_id = $config('req.customizer.user_id')) {
            $this->user = $user;
            $session->set('user', Factory::getUser($user_id));
        }
    }

    public function prepareContent(Config $config, CMSApplication $application, Session $session, Builder $builder, $event)
    {
    ...
    public static function loadTemplate(Config $config, $event)
    {
        ...
        if ($article->params->get('access-edit') && !$config('app.isCustomizer')) {
            $url = Route::_(RouteHelper::getFormRoute($article->id) . '&return=' . base64_encode(Uri::getInstance()));
            $content .= "<a style=\"position: fixed!important\" class=\"uk-position-medium uk-position-bottom-right uk-button uk-button-primary\" href=\"{$url}\">" . Text::_('JACTION_EDIT') . '</a>';
        }
